...
class ApiService {
    ...
    return data.map((item) {
      try {
        ...
        return ActivityItem(
          ...
          type: 'error',
          action: '',
          content: _extractContent(item) ?? 'Failed to load activity content',
          primaryLink: '',
          itemId: '0',
          secondaryItemId: '0',
  ...
  Future<ActivityComment> updateComment(String commentId, String content) async {
    try {
      final apiUrl = dotenv.env['API_URL'];
      final response = await http.put(
  ...
  Future<ActivityComment> updateNestedReply(String nestedReplyId, String content) async {
    try {
      final apiUrl = dotenv.env['API_URL'];
      final response = await http.put(
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to update nested reply: ${response.statusCode}');
      }
    } catch (e, stackTrace) {
